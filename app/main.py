...
async def presets_roles():
    ...
    Presets carry no hardcoded model: each one inherits the configured
    default (ANTHROPIC_MODEL env var) at response time, so the env override
    advertised in .env.example actually governs them (issue #13).
    """
    default_model = get_default_model()
    return [{**r, "model": default_model} for r in PRESET_ROLES]
